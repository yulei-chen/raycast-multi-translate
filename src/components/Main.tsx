...
export function Main(): ReactElement {
  ...
  const sourceText = rawSourceText
    .replace(langReg, (_, lang) => {
      ...
      if (_lang !== inlineLangFrom) setInlineLangFrom(_lang)
      return ""
    })
    .trim()
    ...
    <List
      ...
      searchBarAccessory={
        <List.Dropdown
          ...
          value={langFrom}
          onChange={(v) => {
            if (v === "auto") setUserLangFrom(undefined)
            else setUserLangFrom(v as LanguageCode)
          }}
        >
          <List.Dropdown.Item
            key="auto"
            title={
              singleSource ? `Auto (${getLanguageName(fromLangs[0])})` : "Auto"
            }
            value="auto"
          />
          ...
          <List.Dropdown.Section>
            {Object.values(languages)
              .filter(
                (lang) =>
                  lang.code !== "auto" && !targetLanguages.includes(lang.code),
              )
              .map((lang) => (
                <List.Dropdown.Item
                  key={lang.code}
    ...
    >
      {results?.map((item, index) => {
        const webDicts = webDictionaries
          .filter(
            (dic) =>
              (dic.sentence || !item.translated.includes(" ")) &&
              (!dic.enables ||
                dic.enables.some((i) => targetLanguages.includes(i))) &&
              [item.from, item.to].includes(dic.lang),
          )
          .map((dic) => {
            const text =
              item.from === dic.lang ? item.original : item.translated
            return {
              title: dic.name,
              ...
              <ActionPanel>
                <ActionPanel.Section>
                  ...
                  {webDicts.map((dic, idx) => (
                    <Action.OpenInBrowser
                      shortcut={
                        idx ? undefined : { modifiers: ["cmd"], key: "enter" }
                      }
                      key={dic.title}
